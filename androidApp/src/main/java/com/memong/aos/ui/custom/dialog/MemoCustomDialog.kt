...
import com.memong.aos.databinding.DialogMemoCustomBinding

internal class MemoCustomDialog(context: Context) : Dialog(context) {

    private val binding: DialogMemoCustomBinding =
        DialogMemoCustomBinding.inflate(LayoutInflater.from(context))

    companion object {
        const val NAME = "MemogCustomDialog"
    }
    ...
    fun setMessageHtml(message: CharSequence) {
        binding.tvMessage.text = message
        binding.tvMessage.visibility = if (message.isNotEmpty()) View.VISIBLE else View.GONE
    }
